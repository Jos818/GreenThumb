...
public class DialogueManager : MonoBehaviour
{
    ...
    */
    ...
    private void PrintDialogue()
    {
        ...
        else if (inputStream.Peek().Contains("[PITCH=")) //Set the name of the speaker
        {
            string part = inputStream.Peek();
            string pitch = inputStream.Dequeue().Substring(part.IndexOf('=') + 1, part.IndexOf(']') - (part.IndexOf('=') + 1));
            int pitchnum = Convert.ToInt32(pitch);
            if (pitchnum == 1)
            {
                audio.clip = high;
            }
        ...
        /*else if (inputStream.Peek().Contains("[RESUMETIMELINE"))
        {
            timelineController.Resume();
        }
        else if (inputStream.Peek().Contains("[PAUSETIMELINE"))
        {
